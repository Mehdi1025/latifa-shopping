...
import type { SalesHeatmapPayload } from "@/lib/server/sales-heatmap";
...
export function SalesHeatmap() {
  ...
  const hourStart = payload?.hourStart ?? 9;
  const numHours = payload?.numHours ?? 11;
    ...
    <div className="w-full">
      <div className="mb-5 flex flex-wrap items-baseline justify-between gap-2">
        <p className="text-[11px] font-medium uppercase tracking-[0.14em] text-neutral-400">
          Météo des ventes (30 derniers jours)
        </p>
        {payload && !loading ? (
      ...
      {loading ? (
        ...
        <div className="rounded-xl border border-dashed border-slate-200 bg-slate-50/80 px-4 py-10 text-center">
          <p className="text-sm font-medium text-slate-700">En attente de vos premières ventes</p>
          <p className="mt-1 text-xs text-slate-500">
            La carte se remplira automatiquement dès que des ventes seront enregistrées en caisse.
          </p>
        </div>
      ) : (
        <>
              ...
              {matrix.map((row, d) => (
                <div key={d} className="contents">
                  ...
                  {row.map((cell, h) => {
                    ...
                    const saleLabel =
                      cell.count === 1 ? "1 vente" : `${cell.count} ventes`;
                    const title =
                      cell.count === 0
                        ? `${dayName} ${slot} — aucune vente`
                        : `${saleLabel} le ${dayName} à ${slot}`;
