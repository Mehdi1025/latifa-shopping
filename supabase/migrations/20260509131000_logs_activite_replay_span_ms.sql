-- Durée Δ (ms) rrweb entre min(timestamp) et max(timestamp), alignée avec lib/rrwebReplay.ts.
-- Filtre liste VAR (~500 ms) : MIN_RRWEB_REPLAY_SPAN_MS côté appli doit rester cohérent.
