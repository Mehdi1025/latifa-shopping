...
import { Search, ScanLine, ShoppingBag, ShoppingCart } from "lucide-react";
...
export default function StockPage() {
  ...
  const tiroirOuvertRef = useRef(false);
  tiroirOuvertRef.current = modeleTiroir !== null;
  ...
  const ouvrirParEan = useCallback(
    (raw: string) => {
      if (tiroirOuvertRef.current) return;
  ...
  return (
    <div className="flex h-[100dvh] max-h-[100dvh] min-h-0 w-full max-w-full flex-1 flex-col overflow-hidden overscroll-none bg-gray-50 select-none max-md:max-w-[100vw] [&_a]:[-webkit-tap-highlight-color:transparent] [&_button]:[-webkit-tap-highlight-color:transparent]">
      <div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden">
        <div className="z-20 w-full max-w-full shrink-0 border-b border-gray-200/80 bg-gray-50/95 backdrop-blur-md max-md:sticky max-md:left-0 max-md:right-0 max-md:top-[calc(3.5rem+env(safe-area-inset-top,0px))] md:border-gray-100 md:shadow-sm">
          <div className="px-4 pb-2 pt-2 md:px-7 md:pb-5 md:pt-6 lg:px-10 lg:pb-6">
            <div className="mb-3 flex flex-wrap items-start justify-between gap-2 md:mb-5">
              <div className="min-w-0">
                <h1 className="text-base font-semibold tracking-tight text-gray-900 md:text-xl lg:text-2xl">
                  📦 État des Stocks
                </h1>
                <p className="mt-0.5 text-[10px] leading-snug text-gray-500 md:mt-1 md:text-sm">
                  Consultation uniquement — scan à lecture unique ; fermez la fiche avant un nouveau
                  scan.
                </p>
              </div>
              <Link
                href="/vendeuse"
                className="inline-flex min-h-11 shrink-0 items-center gap-2 rounded-xl bg-gray-900 px-3 py-2 text-xs font-semibold text-white shadow-sm transition hover:bg-gray-800 active:scale-[0.98] md:px-4 md:text-sm"
              >
            ...
            <div className="mb-2 flex shrink-0 flex-row items-stretch gap-2 md:mb-4 md:gap-3">
              <div className="flex min-h-[48px] min-w-0 flex-1 shrink-0 items-center gap-2 rounded-xl border border-gray-100/90 bg-white px-3 py-2 shadow-sm md:min-h-[52px] md:gap-3 md:ring-1 md:ring-gray-100/80">
                ...
                <input
                  ...
                  aria-label="Recherche stock ou EAN"
                />
              </div>
              <button
                type="button"
                onClick={() => setScannerOpen(true)}
                disabled={modeleTiroir !== null}
                className="flex min-h-[48px] min-w-[48px] shrink-0 items-center justify-center gap-2 rounded-xl bg-gray-900 px-3 text-base font-semibold text-white shadow-md transition active:scale-[0.98] [-webkit-tap-highlight-color:transparent] enabled:hover:bg-gray-800 disabled:cursor-not-allowed disabled:opacity-40 md:min-h-[52px] md:min-w-[11rem] md:px-6 md:py-4"
            ...
            <div className="flex snap-x snap-mandatory gap-2 overflow-x-auto overscroll-x-contain pb-1 [-ms-overflow-style:none] [scrollbar-width:none] md:gap-2 [&::-webkit-scrollbar]:hidden">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain scroll-smooth max-md:px-4 md:px-7 lg:px-8 xl:px-10">
          {loading ? (
            ...
            <div className="grid w-full max-w-full grid-cols-2 gap-2.5 pt-1 pb-8 sm:grid-cols-3 md:grid-cols-2 md:gap-4 md:pt-2 md:pb-10 lg:grid-cols-3 max-md:pb-10">
                ...
                {groupesModelesAffiches.map((groupe) => {
                  ...
                  return (
                    <motion.button
                      ...
                      disabled={modeleTiroir !== null}
                      className="group flex min-h-0 flex-col items-stretch rounded-lg border border-gray-200/90 bg-white p-2.5 text-left shadow-sm transition duration-200 ease-out [-webkit-tap-highlight-color:transparent] active:scale-[0.98] enabled:hover:border-gray-300 disabled:pointer-events-none disabled:opacity-50 md:min-h-[200px] md:rounded-2xl md:p-6 md:shadow-sm lg:rounded-3xl lg:p-7"
                    >
                      ...
                      <div className="mt-2 flex min-h-0 items-end justify-end md:mt-3 md:min-h-10">
                        <span
                          className={`shrink-0 rounded-full border px-2 py-0.5 text-[9px] font-semibold tabular-nums md:px-2.5 md:py-1 md:text-[11px] ${
                            stockTotal > 0
